...
live2d_settings['showLinkMessage'] = true;            //鼠标滑过a标签添加提示
...
function loadTipsMessage(result) {
    ...
    $(document).on("mouseover", 'a', function () {
        var result = window.waifu_tips;
        var linkMessage = result.waifu.link_message.link;
        var href = $(this).attr("href");
        if (href.indexOf('/') < 0) {
            return;
        }
        if (live2d_settings.showLinkMessage) showMessage(linkMessage + href + '吗？', 1000, true);
    })
